...
app.post("/create-shipment", async (req, res) => {
  try {
    ...
    const sub_total = orderItems.reduce(
      (sum, it) =>
        sum + Number(it.selling_price || 0) * Number(it.units || 1),
      0
    );
    ...
    // TEST MODE OR Missing AWB → Dummy AWB
    if (!awbNumber) {
    ...
    // Save to Firebase
    await db
      .ref(`orders/${order.userId}/${order.orderId}`)
      .update({
...
app.get("/track/:awb", async (req, res) => {
  try {
    ...
    if (!awb)
      return res
        .status(400)
        .json({ success: false, message: "Missing awb" });

    // TEST MODE → Fake tracking response
    if (MODE === "test") {
      return res.json({
        success: true,
        data: {
...
app.post("/admin/approve-cancel", async (req, res) => {
  try {
    ...
    await orderRef.update(cancelMeta);
